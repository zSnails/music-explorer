  ...
  <section class="flex flex-col gap-5 min-w-[1008px] w-full">
    ...
    <div class="grid grid-cols-2 gap-2">
      <ClientOnly fallback-tag="span" fallback="Loading artist...">
        <UCard v-for="(artist, idx) in artists?.items" :key="idx" as="a" target="" :href="`/albums/${artist.id}`"
          ...
          <div class="flex flex-row justify-between">
            <img :src="artist.images[0]?.url ?? 'https://store-images.s-microsoft.com/image/apps.10546.13571498826857201.6603a5e2-631f-4f29-9b08-f96589723808.dc893fe0-ecbc-4846-9ac6-b13886604095'" :alt="artist.name" width="100" class="mb-2 rounded-xl">
            <h1>{{ new Intl.NumberFormat("en-US",{notation: "compact"}).format (artist.followers.total)+ " followers"}}</h1> 
          </div>

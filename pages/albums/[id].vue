    ...
    <section class="flex flex-col gap-5 min-w-[1008px] w-full">
        <div class="grid grid-cols-2 gap-2">
            <ClientOnly fallback-tag="span" fallback="Loading albums...">
                <UCard v-for="(album, idx) in albums?.items" :key="idx" as="a" target=""
                    :href="`/albumInfo/${album.id}`"
                    ...
                    <div class="flex flex-row justify-between">
                        <img :src="album.images[0]?.url ?? 'https://store-images.s-microsoft.com/image/apps.10546.13571498826857201.6603a5e2-631f-4f29-9b08-f96589723808.dc893fe0-ecbc-4846-9ac6-b13886604095'"
                            :alt="album.name" width="100" class="mb-2 rounded-xl">
                        <h1>{{ new Intl.NumberFormat("en-US", { notation: "compact" }).format(album.total_tracks) }}
                            tracks</h1>
                    </div>

    ...
    <section class="flex flex-col gap-5 min-w-[1008px] w-full">
        <div class="flex flex-col items-baseline">
            <UInput class="w-full" size="xl" icon="i-heroicons-magnifying-glass-20-solid" v-model="search"
                @change="debouncedSearch" :placeholder="$t('prompt')" :ui="{ icon: { trailing: { pointer: '' } } }">
                <template #trailing>
        ...
        <div class="grid grid-cols-2 gap-2">
            <ClientOnly fallback-tag="span" :fallback="$t('loading-artists')">
                <UCard v-for="(artist, idx) in artists?.items" :key="idx" as="a" target=""
                    :href="localePath(`/artists/${artist.id}`)"
...
const debouncedSearch = useDebounceFn(async () => {
    ...
    if (query !== "") {
        const found = await loadArtists(query);
        artists.value = found;
    }
}, 1000);

async function loadArtists(query: string): Promise<Artists> {
